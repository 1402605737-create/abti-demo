import type { BSideResult, ParseResult, PersonaResult, PromptMessage, UserSelectionsPayload } from '@/types/demo'

function formatSelections(selections: string[]) {
  return selections.join('、')
}

export function buildParsePrompt(input: UserSelectionsPayload): PromptMessage[] {
  return [
    {
      role: 'system',
      content:
...
export function buildBSidePrompt(
  parseResult: ParseResult,
...
export function stringifyPreview(data: ParseResult | PersonaResult | BSideResult | null) {
  if (!data) {
    return '暂无结果'
  }
